...
resource "aws_vpc_peering_connection" "this_vpc_peer" {
  ...
  tags = merge(var.vpc_peer_connection_tags, var.tags, tomap({"Side"= "Requester"}), tomap({"Name" = format("%s.peer_to_%s", var.name, var.peer_vpc_id)}))
}
...
resource "aws_vpc_peering_connection_accepter" "this_vpc_peer_accepter" {
  ...
  tags = merge(var.vpc_peer_connection_tags, var.tags, tomap({"Side"= "Accepter"}), tomap({"Name"= format("%s.peer_to_%s", var.peer_vpc_id, var.name)}))
}
