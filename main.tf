...
resource "aws_vpc" "this" {
  ...
  tags = merge(var.vpc_tags, var.tags, tomap({"Name" = var.name}))
}
...
resource "aws_internet_gateway" "this_inet_gw" {
  ...
  tags = merge(var.inet_gw_tags, var.tags, tomap({"Name"= format("%s.InternetGateway", var.name)}))
}
...
resource "aws_subnet" "this_public_subnet" {
  ...
  tags = merge(var.public_subnet_tags, var.tags, tomap({"Name"= format("%s.subnet_%s", var.name, var.availability_zones[count.index])}))
}
